...
# utility functions for deriving summary info from the CHARIS hypsometry
# files.
...
def ymd_to_date(fields):
    ''' fields is a 3-member tuple '''
    return dt.Date(int(fields[0]), int(fields[1]), int(fields[2]))


class Hypsometry():
    """Hypsometry class manages CHARIS hypsometry data and metadata.  It is
    based on the class written by Mary Jo Brodzik for the CHARIS project.
    ...
    def read(self, filename, verbose=False):
        ...
        if lines[num_comments + 1] != '\n':
            ...
            col_names = ['yyyy', 'mm', 'dd', 'doy'] + [
                int(float(i)) for i in lines[num_comments + 1].split()]

            # Now read the data part of the file into a DataFrame Use
            # header=None in order to not use anything in the file for
    ...
    def write(self, filename, decimal_places=6, verbose=False):
        ...
        # The columns are integers so they will sort numerically
    ...
    def get_contour(self, file=sys.stderr):
        ...
        """
        # Find out the differences between adjoining elevations
        elevations = self.data.columns.values
        differences = elevations - np.roll(elevations, 1)

        # Check that they are all the same and return the
        # contour interval.  The set function is great to
        # find unique elements in a list.  Ignore the
        # "rolled" difference at the beginning
        unique_diffs = set(differences[1:])
        if 1 != len(unique_diffs):
            print(" %s : Unexpected variable contours in hypsometry." %
                  (__name__),
                  file=sys.stderr)
            raise ValueError
    ...
    def unfurl(self, contour_m=100, verbose=False):
        ...
        # Add the highest elevation expected on Earth,
        # Everest is 8,848 m, so calculate the highest contour
        # level that would contain it.
        everest_m = 8848
        highest_col = contour_m * (everest_m // contour_m)

        # Duplicate the highest values for the last column
        self.data[highest_col] = self.data[self.data.columns[-1]]

        for i, col in enumerate(self.data.columns[:-1]):
            elevation_diff = self.data.columns[i+1] - self.data.columns[i]
            num = (elevation_diff // contour_m) - 1
            extra_columns = col + contour_m + (
                np.arange(num) * contour_m)
            for extra_col in extra_columns:
                self.data[extra_col] = self.data[col]

        # Sort the columns by elevation
        self.data.sort_index(axis=1, inplace=True)

        # Make a new index that is the days of a leap year
        # The actual year in the DDFs will be ignored
        # when meltmodels does the multiplication with area and
        # temperature.  Producing an unfurled DDF array with
        # 366 rows ensures that we have enough rows for
        # leap or non-leap years
        year = 2000
        self.data.index = self.data.index.map(
                lambda t: t.replace(year=year))
        year_str = "%4d-01-01" % year
        full_year_date_index = pd.date_range(
            year_str, periods=366, freq='D')
        self.data = self.data.reindex(
            index=full_year_date_index,
            method='ffill')
    ...
    def sum_by_elevband(self, verbose=False):
        """Sum hypsometry data over time, resulting in sums by elevation band
        ...
        return(self.data[self.data.columns].sum(axis=0))
